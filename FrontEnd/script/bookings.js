...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        const userBookings = await apiRequest(`/Bookings/mine/${currentUser.id}`);

        setupFilters(userBookings);
...
function setupFilters(userBookings) {
    ...
    filterButtons.forEach((btn, i) => {
        ...
        button.style.cssText =
            "padding:8px 16px; border-radius:8px; border:1px solid var(--border-color); cursor:pointer; font-weight:500; transition:0.2s;";
        ...
        button.addEventListener("click", () => {
            ...
            const filtered = btn.status
                ? userBookings.filter(b => getBookingStatus(b) === btn.status)
                : userBookings;
    ...
    userBookings.forEach(booking => {
        ...
        bookingCard.innerHTML = `
            <div class="card booking-card shadow-sm h-100">
                <img src="${booking.image || "https://picsum.photos/400/300?random=99"}" alt="${booking.propertyTitle}" class="booking-card-img">

                <div class="card-body d-flex flex-column">
                    <h6 class="card-title mb-2">${booking.propertyTitle}</h6>

                    <div class="detail-label">Location</div>
                    <div class="detail-value">${booking.city}, ${capitalize(booking.country)}</div>

                    <div class="detail-label">Price</div>
                    <div class="detail-value" style="color:red; font-weight:700;">
                    ...
                    <div class="detail-value">${booking.hostName || "Host"}</div>

                    <div class="detail-label">Total</div>
                    <div class="detail-value">€${booking.totalAmount}</div>

                    <div class="detail-label">Payment</div>
                    <div class="detail-value">${booking.paymentMethod || "-"} (${booking.paymentStatus || "-"})</div>

                    <div class="mt-auto pt-3">
                        <span class="badge ${badgeColor} mb-2">${status}</span>

                        <div class="card-buttons" style="display:flex; gap:8px;">
                            <button class="view-details-btn" style="background-color:blue; flex:1;">
                                View Details
                            </button>
                        ...
                        ${status === "Stayed"
                            ? `<button class="review-btn" style="width:100%; margin-top:8px; padding:8px; background:var(--button-color); color:white; border:none; border-radius:8px; cursor:pointer; font-weight:500;">⭐ Write a Review</button>`
                            : ""}
                    </div>
                </div>
            </div>
        ...
        bookingCard
            ...
            .addEventListener("click", () => openBookingModal(booking));
        ...
        if (reviewBtn) {
            reviewBtn.addEventListener("click", () => openStarReviewModal(booking));
        }
...
async function cancelBooking(bookingId) {
    ...
    try {
        const result = await apiRequest(`/Bookings/cancel/${bookingId}`, "DELETE");
        ...
        const updatedBookings = await apiRequest(`/Bookings/mine/${currentUser.id}`);
...
function openStarReviewModal(booking) {
    ...
    stars.forEach(star => {
        ...
        star.addEventListener("click", () => {
            ...
            modal.querySelector("#starLabel").textContent =
    ...
    modal.querySelector("#submitStarReview").addEventListener("click", async () => {
        ...
        try {
            ...
            const result = await apiRequest("/Reviews/create", "POST", {
...
function openBookingModal(booking) {
    ...
    const country = capitalize(booking.country);
    const status = getBookingStatus(booking);
    ...
    document.getElementById("bookingModalDesc").textContent =
        `Your reservation in ${booking.city}. Check-in: ${formatDate(booking.checkInDate)} | Check-out: ${formatDate(booking.checkOutDate)}`;
    ...
    document.getElementById("bookingModalPrice").textContent = `€${booking.pricePerNight} / night`;
    document.getElementById("bookingModalGuests").textContent = `${booking.guests || 1} guest(s)`;
    document.getElementById("bookingModalOwner").textContent = booking.hostName || "Host";

    currentBookingImages = booking.image
        ? [booking.image]
...
function updateBookingImageCounter() {
    document.getElementById("bookingImageCounter").textContent =
        `${currentBookingImageIndex + 1}/${currentBookingImages.length}`;
}
